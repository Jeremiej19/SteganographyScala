object ArgumentParser {
  ...
  private val FILE_WITH_SECRET_FILE_NAME: String = "out"
  private val SECRET_FILE_NAME: String = "secret"
  private val DEFAULT_OUTPUT_FILE_EXTENSION: String = "png"
  private val NO_ENOUGH_PARAMETERS_ERROR: String = "No enough parameters!"
  private val LOGS_ENABLED = false
  def parse(command: String, a: String*): Unit = {
    command match
      case "hide" =>
        try {
          val args = a.toArray
          val baseFilePath: String = args(0)
          val secretFilePath = args(1)
          val outputFilePath :String =
            if ( args.length == 2 )
             FILE_WITH_SECRET_FILE_NAME+"."+DEFAULT_OUTPUT_FILE_EXTENSION
            else
              args(2)
          hideImage(baseFilePath, secretFilePath, outputFilePath)
        } catch {
          case ex: ArrayIndexOutOfBoundsException =>
            println(NO_ENOUGH_PARAMETERS_ERROR)
            if (LOGS_ENABLED) ex.printStackTrace()
        }

      case "hide_text" =>
        try {
          val args: Array[String] = a.toArray
          val baseFilePath: String = args(0)
      ...
      case "show" =>
        try {
          ...
          val outputFilePath: String =
            if (args.length == 1)
              SECRET_FILE_NAME + "." + DEFAULT_OUTPUT_FILE_EXTENSION
            else
              args(1)
  ...
  def hideImage(baseFilePath: String, secretFilePath: String, outputFilePath: String): Unit = {
    ...
    ImageIO.write(photo3, getExtension(outputFilePath), new File(outputFilePath))
  }
  ...
  def showImage(secretFilePath: String, outputFilePath: String): Unit = {
    ...
    ImageIO.write(photo4, getExtension(outputFilePath), new File(outputFilePath))
  }
